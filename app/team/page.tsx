...
export default function TeamPage() {
  ...
  const fetchData = async () => {
    try {
      const [membersRes, streamersRes] = await Promise.all([
        fetch('/api/team'),
        fetch('/api/streamers'),
      ])

      const [membersData, streamersData] = await Promise.all([
        membersRes.json().catch(() => []),
        streamersRes.json().catch(() => []),
      ])

      setMembers(Array.isArray(membersData) ? membersData : [])
      setStreamers(Array.isArray(streamersData) ? streamersData : [])
      // Voice actors şimdilik boş array olarak bırakılıyor
      setVoiceActors([])
    } catch (error) {
        ...
        {allMembers.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {allMembers.map((member) => (
              <div
                key={member.id}
                className="group bg-white rounded-2xl shadow-lg hover:shadow-xl border border-gray-200 hover:border-blue-300 transition-all duration-200 overflow-hidden relative"
              >
                <Link
                  href={
                    member.type === 'team' 
                ...
                <div className="absolute top-4 right-4 z-10">
                  <DeleteButton
                    id={member.id}
                    type={member.type === 'team' ? 'team' : member.type === 'streamer' ? 'streamer' : 'team'}
                    onDelete={fetchData}
                    compact={true}
                  />
                </div>
