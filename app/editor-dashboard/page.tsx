...
export default function EditorDashboardPage() {
  ...
  const loadWorks = async (editorId: string) => {
    try {
      const res = await fetch(`/api/team/${editorId}/tasks`, { credentials: 'include' })
      const data = await res.json()
      if (res.ok) {
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Hoş geldiniz, {editor.name}</h1>
              <p className="text-gray-600">Kurgu ve kazanç bilgilerinizi buradan takip edebilirsiniz</p>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border">
          <div className="p-6 border-b">
            <h2 className="text-lg font-semibold text-gray-900">Son Kurgularım</h2>
          </div>
          <div className="p-6">
            {works.length === 0 ? (
