...
import { Film, Calendar, DollarSign, User, CheckCircle, Clock, Edit, Trash2, Save, X } from 'lucide-react'
...
export default function EditWorksPage() {
  const [works, setWorks] = useState<EditWork[]>([])
  ...
  const fetchWorks = async () => {
    try {
      ...
      if (res.ok) {
        ...
        toast.error('Video edit işleri yüklenemedi')
      }
    } catch (error) {
      console.error('Error fetching edit works:', error)
  ...
  const handleEditPrice = (work: EditWork) => {
    setEditingId(work.id)
    setEditPrice(work.editPrice || 0)
  }
  ...
  const handleSavePrice = async (workId: string) => {
    try {
      const res = await fetch(`/api/content-registry/${workId}`, {
        ...
        body: JSON.stringify({ editPrice: editPrice })
      })
  ...
  const totalAmount = works.reduce((sum, w) => sum + (w.editPrice || 0), 0)
  const paidAmount = works.filter(w => w.editPaid).reduce((sum, w) => sum + (w.editPrice || 0), 0)
  const unpaidAmount = works.filter(w => !w.editPaid).reduce((sum, w) => sum + (w.editPrice || 0), 0)
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-4xl font-bold">Video Edit İşleri</h1>
              </div>
              <p className="text-green-100">Video editörlerin yaptığı işleri yönetin</p>
            </div>
            <div className="text-right">
              <p className="text-green-200 text-sm mb-1">Toplam İş</p>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
          ...
          {works.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500">Henüz video edit işi yok</p>
            </div>
          ) : (
                    ...
                    <tr key={work.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            <p className="text-sm font-medium text-gray-900">{work.editor.name}</p>
                          </div>
                        </div>
                      </td>
                        ...
                        {editingId === work.id ? (
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className="w-28 px-3 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                              placeholder="Ücret"
                            />
                            <button
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold text-gray-900">
                              {work.editPrice ? `${work.editPrice.toLocaleString('tr-TR')} ₺` : '-'}
                            </span>
                            <button
