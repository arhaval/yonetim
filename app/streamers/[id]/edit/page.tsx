...
export default function EditStreamerPage() {
    ...
    fetch(`/api/streamers/${streamerId}`)
      ...
      .then((data) => {
        if (data) {
          ...
          setTeamRates(
            data.teamRates?.map((tr: any) => ({
              teamName: tr.teamName,
              hourlyRate: tr.hourlyRate?.toString() || '0',
            })) || []
          )
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/streamers/${streamerId}`, {
        method: 'PUT',
  ...
  return (
    <Layout>
      <div className="px-4 py-6 sm:px-0">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Yayıncı Düzenle</h1>
        </div>
                        ...
                        <tr key={index}>
                          <td className="px-3 py-2 whitespace-nowrap">
                            <input
                              ...
                              placeholder="Eternal Fire, Sangal, Arhaval..."
